...
namespace VolatileAIO.Organs.Brain.Cars
{
    public class Volkswagen
    {
        ...
        public static void deathWalk(Vector3 goalPosition, AttackableUnit target = null, bool noMove = false)
        {
            ...
            if (CanMove() && !noMove)
            {
                if (target != null && (CurrentMode == Mode.Lasthit || CurrentMode == Mode.Harass))
                    _killUnit = target;
                if (_killUnit != null && !(_killUnit is AIHeroClient) && _killUnit.IsValid && !_killUnit.IsDead && _killUnit.Position.Distance(Player.Position) > getRealAutoAttackRange(_killUnit) - 30)//Get in range
        ...
        private static Obj_AI_Base GetBestHeroTarget(out bool soliderHit)
        {
            ...
            soliderHit = false;
            return hitsToKill < 4 ? killableEnemy : TargetSelector.GetTarget(Player.AttackRange + Player.BoundingRadius, DamageType.Physical);
        }
        ...
        {
            var after = Player.AttackDelay * 1000 + _lastAutoAttack - Now + _orbwalkerMenu["att"].Cast<Slider>().CurrentValue;
        ...
        {
            var after = _lastAutoAttackMove + Player.AttackCastDelay * 1000 - Now + _orbwalkerMenu["mov"].Cast<Slider>().CurrentValue + ((HyperCharged()) ? 150 : 0);
        ...
        public static void AddToMenu()
        {
            ...
            _orbwalkerMenu.Add("att", new Slider("Attack Delat", 0, -100, 250));
        ...
        {
            return AzirSoldiers.Where(s => s.IsValid && !s.IsMoving && !s.IsDead && !s.IsMoving && ObjectManager.Player.Distance(s) <= 875 * 875 /*(!Animations.ContainsKey(s.NetworkId) || Animations[s.NetworkId] != "Inactive")*/).ToList();
        }
